...
void CCellmovingRowSegment::printRow( CPlaceDB& db,const double& cutLine)
{
	...
	for(iter=m_rowmodule.begin(); iter!=m_rowmodule.end(); ++iter)
	{
		cerr<<"["<<iter->first<<","<<iter->second<<", mx:"<<db.m_modules[iter->second].m_x <<"] ";
	}
}
void CCellmovingRowSegment::updateRowModule(const double& cutLine, CPlaceDB& db, bool isRight,  const double& x,const double& length)
{
...
//		assert(db.m_modules[iter->second].m_x==iter->first);
		...
		db.SetModuleLocation(iter->second,newx,db.m_modules[iter->second].m_y);
		bool stop=false;
...
//		assert(db.m_modules[iter->second].m_x==iter->first);
		...
		double newx=cutLine-(db.m_modules[iter->second].m_width-length);
		newmap[newx]=iter->second;
		eraseSet.insert(iter->first);
		db.SetModuleLocation(iter->second,newx,db.m_modules[iter->second].m_y);
		bool stop=false;
		...
		if(iter!=m_rowmodule.begin())
		{
			while(stop==false)
			{
				...
				if(iter==m_rowmodule.begin())
				{
					if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)>xFlag)
					{
		//				assert(iter->first==db.m_modules[iter->second].m_x);
				...
				else
				{
					if(iter->second>=(int)db.m_modules.size())
					{
						...
						cerr<<"Warning:: "<<iter->first<<" second: "<<iter->second<<"\n";
					}
					if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)>xFlag)
					{
		//				assert(iter->first==db.m_modules[iter->second].m_x);
...
void CCellmovingRowSegment::findFirstModule(const double& cutLine, CPlaceDB& db, bool isRight, double& step, double& x,double& length)
{
	if(isRight==true) //find the first module to move in the LEFT side of the cutLine
	{
		double res=step;
		map<double,int>::iterator iter=m_rowmodule.lower_bound(cutLine);
		...
		else
		{
			--iter;
			bool stop=false;
			if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)>=cutLine)
			{
...
//					x=db.m_modules[iter->second].m_x;
					x=iter->first;
					length=cutLine-db.m_modules[iter->second].m_x-step;
					return;
				}
				else
				{
...
//						x=db.m_modules[iter->second].m_x;
	...
	else //find the first module to move in the RIGHT side of the cutLine
	{
		...
		if(iter!=m_rowmodule.begin()) 
		{
			...
			if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)>cutLine )
			{
				if( ((db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)-cutLine)>=step)
				{
					//x=db.m_modules[iter->second].m_x;
					x=iter->first;
					length= (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)-cutLine-step;
					return;

				}
				else
				{
					res-=((db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)-cutLine);
					//x=db.m_modules[iter->second].m_x;
					x=iter->first;
					length=0;
				}
...
void CCellmovingRowSegment::shiftCell(const double& cutLine,  CPlaceDB& db, bool isRight, double& step)
{
	double x=0;
	double length=0;
...
void CCellmovingRowSegment::findEmptySpace(const double& cutLine, CPlaceDB& db,  bool isRight, double& empty)
{
	empty=0;
	...
	if(isRight==true) //find the empty space in the right side of cutLine
	{
		...
		map<double,int>::iterator iter=m_rowmodule.upper_bound(cutLine);
		if(iter==m_rowmodule.end())
		{
			if(iter!=m_rowmodule.begin())
			{
				...
				if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)>cutLine )
				{
					moduleLength+=(db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)-cutLine;
				}
				++iter;
			}
			empty=(this->m_absXstart+this->m_length)-cutLine-moduleLength;
			return;
		}
	...
	else //find the empty space in the left side of cutLine
	{
		...
		map<double,int>::iterator iter;
		for(iter=m_rowmodule.begin(); iter!=m_rowmodule.lower_bound(cutLine); ++iter)
		{
			if( (db.m_modules[iter->second].m_x+db.m_modules[iter->second].m_width)<=cutLine)
			{
				moduleLength+=db.m_modules[iter->second].m_width;
...
bool CCellmovingRowSegment::insert_module(CPlaceDB& db, int mID)
{
	...
	{
		cerr<<"\n mID:"<<mID<<" x:"<<db.m_modules[mID].m_x<<" mabsXst:"<<m_absXstart<<" length:"<<m_length<<" +value"<<(this->m_absXstart+this->m_length)<< endl;
...
double CCellmoving::redistributeCellUnitRun(double targetUtil,bool isGoRight)
{
...
//			cout<<"\nRight move, HPWL="<<wl2<<", HPWL with Penalty="<<wl2p;
		    printf( "\nRight move, penalty = %.2f \t(HPWL = %.0f)\t ,(HPWL with penalty = %.0f)", penalty2, wl2,wl2p );
		}
		else
		{
		    printf( "\nLeft  move, penalty = %.2f \t(HPWL = %.0f)\t ,(HPWL with penalty = %.0f)", penalty2, wl2,wl2p );
		}
	}
	return wl2p;
}
...
//	m_pDB->Align();
	if(showMsg==true)
	{
		...
		printf( "\nCR Final: penalty = %.2f \t(HPWL = %.0f)\t ,(HPWL with penalty = %.0f)", penalty2, wl2,wl2p );
...
void CCellmoving::insertModule(int mID)
{
	...
	int y_pos=(int)((my-m_pDB->m_coreRgn.bottom)/m_pDB->m_rowHeight);
//	assert( (y_pos*m_pDB->m_rowHeight+m_pDB->m_coreRgn.bottom)==m_pDB->m_modules[mID].m_y); //the modules should be on the row
	...
	int x_pos;
	map<double,int>::iterator it=m_coreRowsMap[y_pos].upper_bound(mx);

	if(it==m_coreRowsMap[y_pos].begin())
	{
		cerr<<"\nWarning: module:"<<mID<<" 's x coordinate are out of site!!";
	}
	else
	{
		--it;
		x_pos=it->second;
		if(!(m_pDB->m_modules[mID].m_x<=(m_coreRows[y_pos][x_pos].m_absXstart+m_coreRows[y_pos][x_pos].m_length)))
		{
			cerr<<"\n mID:"<<mID<<" x:"<<m_pDB->m_modules[mID].m_x<<" mabsXst:"<<m_coreRows[y_pos][x_pos].m_absXstart<<
				" length:"<<m_coreRows[y_pos][x_pos].m_length<<" +value"<<(m_coreRows[y_pos][x_pos].m_absXstart+m_coreRows[y_pos][x_pos].m_length)<< endl;
			cerr<<"ypos"<<y_pos<<" bottom:"<<m_pDB->m_coreRgn.bottom<<" rowHeight"<<m_pDB->m_rowHeight<<endl;
...
double CCellmoving::findFlux( double target, int binX1, int binY1, int binX2, int binY2, CPlaceBin& placeBin)
{
	if( (placeBin.m_binSpace[binX1][binY1]==0) || (placeBin.m_binSpace[binX2][binY2]==0) )
	{
		return 0;
	}
	else
	{
		...
		double util1=placeBin.m_binUsage[binX1][binY1]/placeBin.m_binSpace[binX1][binY1];
		double util2=placeBin.m_binUsage[binX2][binY2]/placeBin.m_binSpace[binX2][binY2];
		...
		{
			double residual= (placeBin.m_binUsage[binX1][binY1]-targetUsage1) + (placeBin.m_binUsage[binX2][binY2]-targetUsage2);
			if(residual<=0) //bin2 has enough space 
			{
				return placeBin.m_binUsage[binX1][binY1]-targetUsage1;

			}
			double idealUsage1=targetUsage1+residual*(placeBin.m_binSpace[binX1][binY1]/(placeBin.m_binSpace[binX1][binY1]+placeBin.m_binSpace[binX2][binY2]));
...
void CCellmoving::moveCell( double flux, int binX1, int binY1, int binX2, int binY2, CPlaceBin& placeBin)
{
	/////////////////////////////////////////////////////
	...
	int rowPosEnd=findRowPos(placeBin.GetBinY( binY1 )+placeBin.GetBinHeight() );
	...
	for(int i=rowPosStart; i<rowPosEnd; i++)
	{
		...
		{
			--it;
			if( (it->first<cutLine) &&( (m_coreRows[i][it->second].m_length+it->first)>cutLine ) )
			{
...
//			assert(s>0);
			moveSegmentCell(s, interBinRowsID[i], interBinRowsSegmentID[i],cutLine,isGoRight, areaGain);
			if(areaGain==0)
			{
				blackList[i]=true;
			}
...
void CCellmoving::moveSegmentCell(const double& targetStepXmove, const int& rowID, const int& segID, const double& cutLine, const bool& isGoRight, double& areaGain)
{
	...
	double empty=0;
	m_coreRows[rowID][segID].findEmptySpace(cutLine,*m_pDB,isGoRight,empty);
	if(empty==0)
	{
		areaGain=0;
		return;
	...
	else
	{
		...
		m_coreRows[rowID][segID].shiftCell(cutLine,*m_pDB,isGoRight,realStep);
...
void CCellmoving::OutputDensityGrid( double target, string filename,CPlaceBin& placeBin )
{
	...
	for( unsigned int i=0; i<placeBin.m_binSpace[0].size(); i++ )
    {
		for( unsigned int j=0; j<placeBin.m_binSpace.size(); j++ )
		{
			...
			{
				density=placeBin.m_binUsage[j][i]/placeBin.m_binSpace[j][i]-target;
				if(density<0)
					density=0;
			}
			out << density << " ";
...
void CCheckLegal::check()
{
	double t1=seconds();
	cerr<<"\n========= Start Overlapping checking========binSize:"<<this->m_binSize;
	int notInSite=0;
	int notInRow=0;
	int overLap=0;
	///////////////////////////////////////////////////////
	...
	//2.1 build bin
	//vector< map<int,int> > module2bins;
	//module2bins.resize(m_pDB->m_modules.size());
	...
	for(unsigned int k=0; k<m_pDB->m_modules.size(); ++k)
	{
		...
		//if( (binStartX<0) || (binStartX>=m_binSize) || (binEndX<0) || (binEndX>=m_binSize)||
		//	(binStartY<0) || (binStartY>=m_binSize) || (binEndY<0) || (binEndY>=m_binSize))
		//{
		//	cerr<<"\nSx:"<<binStartX<<" Ex:"<<binEndX<<" Sy:"<<binStartY<<" Ey:"<<binEndY;
		//	cerr<<"\nWarning: cell:"<<m_pDB->m_modules[k].m_name<<"("<<m_pDB->m_modules[k].m_x<<
		//		","<<m_pDB->m_modules[k].m_y<<","<< m_pDB->m_modules[k].m_width<<","<<m_pDB->m_modules[k].m_height<<")\n";
		//}
	...
	cerr<<"\nFinish build bins";
	//2.2 fow all module, check overlapping inside bin
			...
			for(int n=binStartY; n<=binEndY; ++n)
			{
				...
				for(int i=0; i<(int)m_moduleBins[m][n].size(); ++i)
				{
					if(m_moduleBins[m][n][i]!=k)
					{
						...
						double area=getOverlapArea((double)m_pDB->m_modules[k].m_x, (double)m_pDB->m_modules[k].m_y,
								(double)m_pDB->m_modules[k].m_x+(double)m_pDB->m_modules[k].m_width, (double)m_pDB->m_modules[k].m_y+(double)m_pDB->m_modules[k].m_height,
								(double)m_pDB->m_modules[mID].m_x, (double)m_pDB->m_modules[mID].m_y,
								(double)m_pDB->m_modules[mID].m_x+(double)m_pDB->m_modules[mID].m_width, (double)m_pDB->m_modules[mID].m_y+(double)m_pDB->m_modules[mID].m_height );
						if(abs( area ) > 0.1)
						{
							cerr<<"\nWarning: cell:"<<m_pDB->m_modules[k].m_name<<"("<<m_pDB->m_modules[k].m_x<<
								","<<m_pDB->m_modules[k].m_y<<","<< m_pDB->m_modules[k].m_width<<") overlap with cell "<<m_pDB->m_modules[mID].m_name<<
								"("<<m_pDB->m_modules[mID].m_x<<","<<m_pDB->m_modules[mID].m_y<<","<< m_pDB->m_modules[mID].m_width<<")!!Area:"<<area<<"";
							++overLap;
