...
void CParserLEFDEF::WriteComponents( ostream& out, CPlaceDB& fplan )
{
    ...
    {
	if( fplan.m_modules[i].m_type >= 0 )
	{
	    nCount++;
	    if( fplan.m_modules[i].m_type >= (int)m_modules.size() )
	    {
		printf( " %d  of %d \n", fplan.m_modules[i].m_type,  m_modules.size() );
	    }
	    assert( fplan.m_modules[i].m_type < (int)m_modules.size() );
	}
    }
	...
	if( fplan.m_modules[i].m_type >= 0 )
	{
	    out << "- " << fplan.m_modules[i].GetName() 
		<< " " << m_modules[ fplan.m_modules[i].m_type ].GetName();
