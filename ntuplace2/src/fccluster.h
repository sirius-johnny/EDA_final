...
private:
	...
	void init2();
	void mergeModule(int mID1,int mID2);
	void updateAffinity(int mID1,int mID2);
	void updateMaxAffinity(int mID);

	void updateAllAffinity(int mID);

//	int chooseMaxAffinity(int mID, double maxClusterArea, double& aff);
//	int findBestAffinity(double maxClusterArea, int& target);
	

//	vector< map<int,set<int> > > m_moduleNeighbor;
	vector< map<int,vector<int> > > m_moduleNeighbor;
	vector< map<int,double> > m_moduleAffinityMap;
