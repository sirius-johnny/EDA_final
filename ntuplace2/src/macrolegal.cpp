...
CMacroLegal::CMacroLegal( CPlaceDB& db,int thresh_row_high , int _adj_run)
{
	m_pDB = &db;
...
// 			   m_pDB->m_modules[i].GetHeight() > theshold * _Rgn_height     )	
	if(( (m_pDB->m_modules[i].m_isFixed ) && (!m_pDB->m_modules[i].m_isOutCore )) || (isMacro(i) &&(!m_pDB->m_modules[i].m_isCluster)))
		{
...
// 	}
    ...
    fprintf( out, "plot[:][:] '-' w l 4, '-' w l 6, '-' w l 2, '-' w l 3\n\n" ); 
...
void CConstraintGraph::OutputSolutionGraph( const char * filename, bool bWithMove )
{
	...
	if (bWithMove)
    		fprintf( out, "plot[:][:] '-' w l 4, '-' w l 3, '-' w l 7\n\n" ); 
    	else
 		fprintf( out, "plot[:][:] '-' w l 4, '-' w l 3\n\n" ); 
