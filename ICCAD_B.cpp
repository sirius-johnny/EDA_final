...
int main(int argc, char *argv[])
{
    ...
    if (fin)
    {
        ...
        while (getline(fin, lineStr))
        {
            ...
            if (words[0] == "TopDieTech")
            {
                TopDieTech = words[1];
            }
            if (words[0] == "BottomDieTech")
            {
                BottomDieTech = words[1];
            }

            if (words[0] == "TerminalSize")
            {
                TerminalSize_X = stoi(words[1]);
                TerminalSize_Y = stoi(words[2]);
            }
            if (words[0] == "TerminalSpacing")
            {
                TerminalSpacing = stoi(words[1]);
            }
            if (words[0] == "TerminalCost")
            {
                TerminalCost = stoi(words[1]);
            }

            if (words[0] == "NumInstances")
            {
