...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<IB.size(); i++){
        cout<<Inst[IB[i]].instName<<", left low (x,y) = ("<<Inst[IB[i]].locationX<<","<<Inst[IB[i]].locationY<<")"<<" ,Rotate R"<<Inst[IB[i]].rotate<<endl;
    }
...
void Output_Format(string filename){
    ...
    for(int i=0; i<IA.size(); i++){
        fout<<"Inst "<<Inst[IA[i]].instName<<" "<<Inst[IA[i]].locationX<<" "<<Inst[IA[i]].locationY<<" "<<"R"<<Inst[IA[i]].rotate<<endl;
    }
    fout<<"BottomDiePlacement "<<IB.size()<<endl;
    for(int i=0; i<IB.size(); i++){
        fout<<"Inst "<<Inst[IB[i]].instName<<" "<<Inst[IB[i]].locationX<<" "<<Inst[IB[i]].locationY<<" "<<"R"<<Inst[IB[i]].rotate<<endl;
    }
